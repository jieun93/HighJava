 ...
 *  	- '6.파일저장' 메뉴를 선택하면 모든 데이터를 파일로 저장한다.
 *  	-  프로그램을 종료할 때 데이터의 변동이 있으면 '저장 후 종료'하도록 처리한다.
 *  
 *  
 *  
 *  
...
public class PhoneBookTest {
	...
	File fileName = new File ("d:/d_other/phoneBook.dat");
	int select;
	...
	//전환번호 정보를 저장하는 메서드
	 void save(){
		...
		try {
			//출렬용 스트림 객체 생성 ==> 객체 저장용
			oos = new ObjectOutputStream(
					new BufferedOutputStream(
							new FileOutputStream(fileName)));
			
			
			oos.writeObject(phoneMap); //map 객체를 저장한다.
...
//	 *  
	...
	private void store() {
		try {
			FileOutputStream  fout = new FileOutputStream("d:/d_other/phoneBook.dat");
