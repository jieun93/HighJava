...
public class RadioButtonTest extends Application {
	...
	public void start(Stage primaryStage) {
		...
		rb1.setUserData("Home");	// 해당 라디오 버튼을 선택 했을때 사용할 값 설정
		...
		group.selectedToggleProperty().addListener(
				new ChangeListener<Toggle>() {
					...
					public void changed(ObservableValue<? extends Toggle> observable,
						...
						// oldValue == > 변경전, newValue ==> 변경 후 
						// 여기에서는 Toggle객체를 RadioButton으로 형변환해서 사용할 수 있다.
						
						
						if(group.getSelectedToggle()!=null) {
							
							// 선택된 RadioButton의  userData 값 구하기
							//String url = group.getSelectedToggle().getUserData().toString();
							
							RadioButton rb = (RadioButton) newValue;
							String url =rb.getText();
							
							Image img = new Image(RadioButtonTest.class.getResourceAsStream("../../images/"+url+".jpg"));
		...
		btn.setOnAction(E->{
			// 라디오 버튼도 isSelected()메서드로 현재 선택 여부를 알 수 있다.
